...
export default function QuizPage() {
  ...
  // FIX: хук теперь НЕ активируется пока joined = false (пока не нажата кнопка)
  const [joined, setJoined] = useState(false);

  const { roomData, updateScore } = useQuizRoom(roomId, nickname, selectedAvatar.emoji, joined);

  const currentQ = roomData?.currentQuestion || 0;

  // FIX: убрали `|| !roomData?.host` — это был главный баг кнопки хоста
  // Теперь хост — ТОЛЬКО тот, чей ник записан в базе
  ...
  // FIX: "Войти" — просто входим, хост остаётся тем кто создал
  const handleJoin = () => {
    setJoined(true);
    setScreen('lobby');
  };

  // FIX: "Создать" — атомарно проверяем есть ли уже хост, если нет — назначаем себя
  const handleCreate = async () => {
    const roomRef = ref(db, `rooms/${roomId}`);
    const snapshot = await get(roomRef);
    const existing = snapshot.val();

    // Назначаем хоста только если комната новая или хоста нет
    if (!existing?.host) {
      ...
      >
        ...
        <div className="bg-slate-800/50 p-8 rounded-[2.5rem] border border-white/10 w-full max-w-md backdrop-blur-xl shadow-2xl">
          ...
          <div className="flex flex-col gap-3">
            {/* FIX: "Войти" — просто подключается к существующей комнате */}
            <button
            ...
            {/* FIX: "Создать" — назначает себя хостом в Firebase */}
            <button
              disabled={!nickname || !roomId}
  ...
  // Лобби
  if (screen === 'lobby') {
    // FIX: ждём загрузки данных из Firebase перед рендером
    if (!roomData) {
      return (
    ...
    // FIX: фильтруем игроков без никнейма — убираем призраков из базы
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6">
        ...
        <div className="grid gap-3 w-full max-w-md mb-10">
          <AnimatePresence>
            {players.map((p: any, idx: number) => (
              <motion.div
                key={p.nickname}  // FIX: используем nickname как ключ, не idx
                initial={{ x: -20, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                exit={{ x: 20, opacity: 0 }}  // FIX: анимация при уходе игрока
                className="bg-slate-800/40 p-5 rounded-2xl flex items-center justify-between border border-white/5 backdrop-blur-md"
              >
                <div className="flex items-center gap-4">
                  <span className="text-4xl bg-slate-900 p-2 rounded-xl">{p.avatar}</span>
                  <span className="text-xl font-bold">{p.nickname}</span>
                  {/* Значок хоста */}
                  {roomData?.host === p.nickname && (
                    <span className="text-xs bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2 py-0.5 rounded-full">Host</span>
                  )}
                </div>
                <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 rounded-full border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-widest">
                  Ready
                </div>
              </motion.div>
            ))}
        ...
        {/* FIX: кнопка только у реального хоста */}
        {isHost ? (
          <button
            onClick={startGlobalGame}
            className="group bg-white text-black p-5 px-16 rounded-2xl font-black text-xl hover:bg-amber-400 transition-all flex items-center gap-3"
          >
            <Play fill="black" /> ПОГНАЛИ!
          </button>
        ) : (
          <p className="text-white/20 text-sm animate-pulse uppercase tracking-widest">
            Ждём хоста...
          </p>
        )}
      </div>
        ...
        <div className="w-full max-w-2xl">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ y: -20, opacity: 0 }}
              className="bg-slate-800/80 p-10 rounded-[2.5rem] border border-white/10 mb-8 shadow-2xl backdrop-blur-xl relative overflow-hidden"
            >
                ...
                className={`p-6 rounded-2xl border-2 text-left font-bold text-lg transition-all ${
                  selectedAnswer === i
                    ...
                    : selectedAnswer !== null && i === q.correctIndex
                      ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400'  // показываем правильный если ошибся
          ...
          {isHost && selectedAnswer !== null && (
            <motion.button
              ...
              className="mt-10 w-full bg-white text-black p-5 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-amber-400 transition-all"
            >
  ...
  if (screen === 'results') {
    const players = roomData?.players
      ? Object.values(roomData.players)
          .filter((p: any) => p.nickname)  // FIX: тоже фильтруем призраков
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6">
        ...
        <div className="w-full max-w-md space-y-3">
          {players.map((p: any, idx: number) => (
            <motion.div
              ...
              className={`p-6 rounded-[2rem] flex items-center justify-between border-2 ${idx === 0 ? 'bg-gradient-to-r from-amber-500/20 to-transparent border-amber-500 shadow-lg' : 'bg-slate-800/30 border-white/5'}`}
            >
              <div className="flex items-center gap-5">
                <span className={`text-2xl font-black italic ${idx === 0 ? 'text-amber-500' : 'text-white/20'}`}>#{String(idx + 1).padStart(2, '0')}</span>
        ...
        <button
          ...
          className="mt-12 text-white/30 uppercase text-xs font-black tracking-[0.3em] hover:text-white transition-all"
        >
          ВЕРНУТЬСЯ В МЕНЮ
        </button>
      </div>
